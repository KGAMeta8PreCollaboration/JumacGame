...
public class LobbyManager : MonoBehaviour
{
	...
	public async void Init()
	{
		DataSnapshot data = await FirebaseManager.Instance.Database.GetReference(
			$"{myTableName}/loginUsers/{FirebaseManager.Instance.Auth.CurrentUser.UserId}").GetValueAsync();
		logInUserData = JsonConvert.DeserializeObject<LogInUserData>(data.GetRawJsonValue());
		DependencyStatus status = await FirebaseApp.CheckAndFixDependenciesAsync();
		if (status == DependencyStatus.Available)
		{
			DatabaseReference reference = FirebaseManager.Instance.Database.GetReference(myTableName);
